...
namespace Caelum
{
	...
	void PointStarfield::addRandomStars (int count)
	{
		for (int i = 0; i < count; ++i) {
			...
			Star s;
			s.RightAscension = Ogre::Degree (float(rasc));
			s.Declination = Ogre::Degree (float(decl));
	...
	void PointStarfield::ensureGeometry ()
	{
        ...
        for (uint i = 0; i < starCount; ++i)
        {
    		...
    		Ogre::Vector3 pos;
		    pos.z = -Math::Cos (Ogre::Degree(float(azm))) * Math::Cos (Ogre::Degree(float(alt)));
		    pos.x =  Math::Sin (Ogre::Degree(float(azm))) * Math::Cos (Ogre::Degree(float(alt)));
		    pos.y = -Math::Sin (Ogre::Degree(float(alt)));
